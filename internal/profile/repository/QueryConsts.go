...
const (
	...
	GetShortProfile    = "SELECT first_name || ' ' || last_name AS name, file_path FROM profile LEFT JOIN file ON profile.avatar=file.id  WHERE profile.id = $1 LIMIT 1;"

	GetCommunitySubs = `WITH subs AS (SELECT profile_id AS id FROM community_profile WHERE community_id = $1) SELECT p.id, first_name, last_name, file_path FROM profile p JOIN subs ON p.id = subs.id JOIN file ON file.id = p.avatar WHERE id > $2 ORDER BY id LIMIT $3;`
)
