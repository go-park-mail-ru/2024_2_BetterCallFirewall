...
const (
	GetProfileByID      = "SELECT id, first_name, last_name, bio, file_path FROM profile LEFT JOIN file ON profile.avatar = file.id WHERE id = $1 LIMIT 1;"
	GetAllProfiles      = "SELECT id, first_name, last_name, file_path FROM profile LEFT JOIN file ON profile.avatar = file.id WHERE id <> $1 ;"
	...
	GetAllSubs          = "WITH subs AS ( SELECT sender AS subscriber FROM friend WHERE (receiver = $1 AND status = 1) UNION SELECT receiver AS subscriber FROM friend WHERE (sender = $1 AND status = -1)) SELECT profile_id, first_name, last_name, file_path FROM profile INNER JOIN subs ON subscriber = profile.id LEFT JOIN file ON profile.avatar = file.id;"
	GetAllSubscriptions = "WITH subscriptions AS ( SELECT sender AS subscription FROM friend WHERE (receiver = $1 AND status = -1) UNION SELECT receiver AS subscriber FROM subscription WHERE (sender = $1 AND status = 1)) SELECT profile_id, first_name, last_name, file_path FROM profile INNER JOIN subscriptions ON subscription = profile.id LEFT JOIN file ON profile.avatar = file.id;"
