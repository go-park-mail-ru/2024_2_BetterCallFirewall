...
const (
	...
	GetFriendsID    = "WITH friendships AS ( SELECT * FROM friend WHERE (sender = $1 OR receiver = $1) AND status = 0) SELECT CASE WHEN sender = $1 THEN receiver ELSE sender END AS friend_id FROM friendships;"
	GetShortProfile = "SELECT first_name || ' ' || last_name AS name, avatar FROM person WHERE id = $1 LIMIT 1;"
)
